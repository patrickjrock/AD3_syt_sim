...
class Rmsd_cal(Analysis):
  ...
  def get_pdb(self, psf): 
    ...
    if fname[:3] == "c2a":
      return "/home/prock/Desktop/AD3_syt_sim/structures/pdb/c2a_average_bound.pdb"
    if fname[:3] == "c2b":
      return "/home/prock/Desktop/AD3_syt_sim/structures/pdb/c2b_average_bound.pdb"
    raise Exception('unreachable')

  def get_selection(self, psf): 
    fname = os.path.basename(psf)
    return "name CA and not resid 271"
    if fname[:3] == "c2a":
      s = "(resid 170:176 and name CA)"
      s = s + "or (resid 230:237 and name CA)"
      return s
    if fname[:3] == "c2b":
